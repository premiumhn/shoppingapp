...
$sql_ventas_por_producto = $pdo->prepare('SELECT p.NombreProducto, SUM(c.Cantidad) as UnidadesVendidas 
...
$sql_ventas_por_producto->bindParam(':PK_Tienda', $_SESSION['PK_Tienda']);
if($filtro_desde != '' && $filtro_hasta != ''){
    $sql_ventas_por_producto->bindParam(':FechaHoraDesde', $filtro_desde); 
    $sql_ventas_por_producto->bindParam(':FechaHoraHasta', $filtro_hasta); 
 }
$sql_ventas_por_producto->execute();
$ventas_por_producto = $sql_ventas_por_producto->fetchAll(PDO::FETCH_ASSOC);
$labels_ventas_productos = [];
for( $i = 0; $i<=count($ventas_por_producto) -1 ; $i++){
	array_push($labels_ventas_productos, $ventas_por_producto[$i]['NombreProducto']);
}
$datos_ventas_productos = [];
for( $i = 0; $i<=count($ventas_por_producto) -1 ; $i++){
	array_push($datos_ventas_productos, $ventas_por_producto[$i]['UnidadesVendidas']);
}
...
<div style="padding:10px 10px 50px 10px;" class="text-center ">
	...
	<a class="btn-volver btn" href="Estadisticas"> <i class="fa fa-arrow-left"></i> Menú estadísticas</a>
		<div class="col-md-12">
		<br>
		<h2 style="background-color: #F8F8F8;border-radius:20px;">Estadísticas productos</h2>
		<br>
			<form action="Estadisticas-Productos" method="GET">
			...
			<div class="cont-chart">
				<h4 style="color: gray;" class="col-md-12">Ventas por producto</h2>
				<br>
				<canvas class=" chart" id="chartVentasPorProducto" width="400" height="400"></canvas>
			</div>
			<br>
			<br>
			<div class="cont-chart">
...
<script type="text/javascript">
	...
	var ctx = document.getElementById('chartVentasPorProducto').getContext('2d');
	var myChart = new Chart(ctx, {
		type: 'bar',
		data: {
			labels: <?php echo json_encode($labels_ventas_productos) ?>,
			datasets: [{
				label: 'Número de ventas',
				data: <?php echo json_encode($datos_ventas_productos) ?>,
